...
const getSoundOptions = (eventKind: EventKind): Array<{
  ref: string;
  label: string;
  recommended: boolean;
}> => {
  const recommendedRefs = getRecommendedEventRoutingSoundRefs(eventKind);
  const recommended = recommendedRefs.map((ref) => ({
    ref,
    label: getEventRoutingSoundCatalogEntry(ref).label,
    recommended: true
  }));
  const recommendedSet = new Set(recommendedRefs);
  const others = eventRoutingSoundCatalog
    .filter((entry) => !recommendedSet.has(entry.ref))
    .map((entry) => ({
      ref: entry.ref,
      label: "catalogOnly" in entry && entry.catalogOnly ? `${entry.label} (catalog only)` : entry.label,
      recommended: false
    }));
