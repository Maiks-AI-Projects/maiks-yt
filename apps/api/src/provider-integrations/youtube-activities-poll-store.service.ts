...
import { createYouTubeLiveChatContextRepository } from "./youtube-live-chat-intake-control-store.service.js";
import type {
...
const resolveActor = async (
  pool: QueryExecutor,
  authUserId: string
): Promise<YouTubeActivitiesPollActor | null> => {
...
export const createYouTubeActivitiesPollRepository = (
  pool: DatabasePool
): YouTubeActivitiesPollRepository => {
  const contextRepository = createYouTubeLiveChatContextRepository(pool);

  return {
    resolveActor: (authUserId) => resolveActor(pool, authUserId),
    resolveSelectedLiveChatContext: () => contextRepository.resolveSelectedLiveChatContext()
  };
};
