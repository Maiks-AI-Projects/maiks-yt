...
- `pnpm dev:visual-smoke` is available for local headless-browser screenshots of key dev testing surfaces. It uses ignored token URLs when available, redacts token query values from generated reports, and writes ignored artifacts under `reports/visual-qa/current-dev-smoke/`. Fresh-browser `/chat` and `/moderation` captures may be marked auth-required until a real signed-in browser/PWA session is used.
- `/admin` now acts as the testing dashboard: grouped admin navigation, `devAuthToken` link preservation through the shared local storage helper, recurring smoke coverage, and live status cards for API health, database health, notifications, provider intake health, and session-admin reachability.
