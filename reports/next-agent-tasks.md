...
- Treat Better Auth trusted origins as a separate auth/security slice; do not casually replace its local and dev defaults without explicit environment-aware tests and senior review.
...
- Michael wants moderators to have their own dedicated control window/PWA, not extra clutter in the creator chat window. First safe scope should combine permitted chat visibility, quick actions, active-rule review/retraction, pending approvals, and helper context behind existing moderator/helper permission gates.
  ...
  - First `/moderation` runtime implementation is deployed and endpoint-smoked on dev: chat-first panel, applied rules, pending approvals, live helper summary, role-filtered quick actions, and signed-in permission enforcement for hide/ban/warn/retract/emergency clear. Manual installed-window moderation smoke remains useful.
