...
- Phase 6A provider library capability mapping is deployed and dev-smoked. It now includes `@twurple/chat` and typed capability states in provider status. Twitch chat library availability is reported separately from runtime state, Twitch EventSub remains gated, Phase 6C generated YouTube OAuth consent/token storage but has not been dev-applied yet, and Discord Gateway/`discord.js` remains gated. Michael wants Discord read-only intake before YouTube because Discord can be validated without going live, while YouTube live chat should stay last among the current chat providers.
...
- Michael wants moderators to have their own dedicated control window/PWA, not extra clutter in the creator chat window. First safe scope should combine permitted chat visibility, quick actions, active-rule review/retraction, pending approvals, and helper context behind existing moderator/helper permission gates.
  ...
  - First `/moderation` runtime implementation is in review: chat-first panel, applied rules, pending approvals, live helper summary, role-filtered quick actions, and signed-in permission enforcement for hide/ban/warn/retract/emergency clear. Deploy/browser smoke remains the coordinator gate.
...
- Phase 6D: Discord read-only Gateway message intake for the configured guild/channel set after adding `discord.js` and confirming required Gateway intents. Keep message content private/control-panel only unless Event Routing later routes it. This should be validated before YouTube because it does not require a live YouTube stream.
