...
- Phase 6G: Provider Connections catalog foundation is implemented locally. It adds a typed provider-event catalog for Twitch EventSub/IRC, YouTube live chat/activity/pubsub, and Discord Gateway/webhook events; a typed event-action catalog; and a read-only `/admin/connections` page listing events, internal triggers, safety badges, and available/gated actions. It intentionally adds no runtime provider subscriptions, webhook/EventSub receivers, provider writes, routing execution, schema changes, money behavior, moderation enforcement, secret edits, Cloudflare/Docker config, or production behavior.
- Phase 6G follow-up: append-only provider intake ledger plus unknown-safe normalization. Generate/review persistence before runtime writes, store offline provider events by default, preserve raw/provider payloads only in a redacted or tightly bounded shape, and route/display events only through explicit Event Routing rules.
