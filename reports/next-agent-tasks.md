...
- Phase 6G: Provider event catalog foundation. Add a typed catalog for Twitch EventSub, YouTube live-chat/activity/pubsub, and Discord Gateway/webhook event names from `ideas/provider-event-intake-inventory-and-internal-triggers.md`; include safety metadata for money-shaped, moderation-shaped, auth/token-shaped, high-volume, and overlay-default-false events. No runtime subscriptions, provider writes, schema changes, or routing behavior.
- Phase 6G follow-up: Always-on provider event intake ledger for non-chat support/community events, starting with append-only history/audit registration and unknown-safe normalization. Store offline provider events by default; route/display them only through explicit Event Routing rules.
