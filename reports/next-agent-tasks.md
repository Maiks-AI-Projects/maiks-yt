...
- Phase 6G next: Twitch EventSub receiver plumbing for verified/log-only events, then additional Discord webhook and YouTube activity/pubsub surfaces. Start with read-only registration/logging of selected low-risk events and unknown-safe fallback rows; do not add provider writes, public routing, money settlement, moderation enforcement, or production behavior.
