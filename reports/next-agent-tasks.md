...
- Issue #15 access recovery: raw `control-dev` URLs with only `accessToken` are expected to fail in fresh/PWA windows because Control Panel tokens still require login after the URL-token gate. Use `/admin`, `/admin/testing`, or `/admin/tokens` from a dev-authenticated web page so links include both `accessToken` and `devAuthToken`; do not weaken Control Panel tokens to `requiresLogin: false`.
...
- Phase A non-provider smoke review adds route-level `/chat` and `/moderation` reachability/injection checks to the dev smoke runner plus `reports/dev-smoke/phase-a-manual-smoke-gap-review.md`. Manual installed-window/token-gated UI smoke is still open, but `/admin/testing` and `reports/dev-manual-testing-guide.md` now include an Installed Window Checklist that recurring and visual smoke both assert. `/admin/testing` also has a local browser-only Manual Testing Checklist with session start tracking / Start new session, Copy progress / Reset marks, Mark section done / Clear section, visible progress/remaining counts, Session Notes included in copied progress, and a Copy template action for the breakage-note template.
  - Quick Open preserves short-lived `devAuthToken` access for admin/tools links and `/account`, so profile and stream-visibility testing can start from the launchpad without losing the test session.
- `pnpm dev:visual-smoke` is available for local headless-browser screenshots of public pages, the active owner admin surface list, standalone `/chat`, `/moderation`, and overlay. It uses ignored token URLs when available, passes `devAuthToken` through the fresh browser, redacts token query values from generated reports, and writes ignored artifacts under `reports/visual-qa/current-dev-smoke/`.
  ...
  - `/admin/testing` now shows that latest recorded readiness evidence in-app, and recurring/visual smoke assert the evidence card remains visible.
...
- `/admin/testing` and the manual testing guide now include explicit money CSV import checks for preview, draft creation, `duplicate_reference`, `possible_duplicate`, and cleanup. Recurring/visual smoke assert that guidance remains visible.
