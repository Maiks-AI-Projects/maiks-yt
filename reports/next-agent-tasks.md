...
- Phase A non-provider smoke review adds route-level `/chat` and `/moderation` reachability/injection checks to the dev smoke runner plus `reports/dev-smoke/phase-a-manual-smoke-gap-review.md`. Manual installed-window/token-gated UI smoke is still open, but `/admin/testing` and `reports/dev-manual-testing-guide.md` now include an Installed Window Checklist that recurring and visual smoke both assert. `/admin/testing` also has a local browser-only Manual Testing Checklist with Copy progress / Reset marks and a Copy template action for the breakage-note template.
