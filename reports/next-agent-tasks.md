...
- YouTube channel identity/selection persistence is implemented for review with generated migration `0021_omniscient_bloodstrike.sql`: saved discovered channels, owner-gated discover-and-save, and set/clear selected live-chat channel. Coordinator still needs to apply the migration on dev, deploy, and smoke with the stored credential.
...
- Phase 6E: YouTube channel identity/selection persistence from the stored owner credential is in review. After it is deployed and smoked, the next YouTube slice is read-only live-chat polling into the private streamer-chat feed. Use YouTube polling interval/backoff, safe token refresh, no overlay routing by default, and no provider writes. Keep polling last among the current chat providers until Michael can live-test it.
