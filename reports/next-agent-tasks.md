...
- Phase 5J completed the docs/design gate for community rules, manual warning/strike escalation, and restriction boundaries. The first public `/community-rules` dev-draft page is implemented and reserved from Page Creator ownership. Final production wording, automatic warnings, real bans, provider enforcement, destructive actions, AI moderation, auth/secrets, money/support authority, production behavior, and new policy/strike schema remain gated.
