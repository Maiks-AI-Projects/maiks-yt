...
- Phase 6G YouTube PubSub subscription manager: local review-ready owner-gated API/UI can show selected-channel topic/callback readiness and request subscribe/unsubscribe with the Google PubSubHubbub hub. The hub does not provide list/status introspection, so this surface reports target readiness and request results only. No provider content writes, public routing, money settlement, moderation enforcement, or production behavior was added.
- Phase 6G next: review/deploy/smoke the YouTube PubSub subscription manager, then add YouTube activities polling. Start with read-only registration/logging and unknown-safe fallback rows; do not add provider writes, public routing, money settlement, moderation enforcement, or production behavior.
