...
- Phase C1 money planning is captured in `reports/money-accounting-stage-plan.md`. Phase C2 generated/applied migration `0023_slimy_harpoon.sql` for private ledger/rules/reports on dev. Phase C3 adds owner-only `/admin/money` manual entry/list foundations. Phase C4 adds owner-only CSV export for the current private ledger list and records export audit metadata. Next money task should improve corrections, receipt references, warnings, period filters, and richer report packages, not public payment behavior.
