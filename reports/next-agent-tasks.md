...
- The default quick temporary mute duration is 10 minutes. Streamer chat options expose `Mute 10m` for fake/local messages only; provider timeouts stay disabled until audited provider-write moderation exists.
...
- Provider action readiness is now visible on `/admin/connections`: Discord/Twitch/YouTube warning sends are implemented fail-closed, while provider-side delete/hide, timeout, and ban actions remain gated with visible reasons. This is read-only catalog/status, not a provider enforcement endpoint.
