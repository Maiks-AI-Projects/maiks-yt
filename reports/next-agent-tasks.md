...
- The homepage stream-status and "Up next" real-schedule integration is code-complete and independently reviewed with zero standards and zero specification findings. It fetches the public schedule once, shares one safe projection between both slots, prefers current live entries with earliest-start tie-breaks, otherwise chooses the earliest valid planned `startsAt` regardless of API array order, omits cancelled/completed entries, and keeps loaded-empty distinct from unavailable or malformed schedule data. `pnpm check:review` passed with 158 Domain, 635 API, 153 Web, Web build, Overlay/Control typechecks, 16 Local Agent, architecture, and diff checks. It is not deployed or browser/live-verified under the desktop stop order.
- The homepage scheduled-game focus slice is code-complete and independently reviewed with zero standards and zero specification findings. It appends the first ordered valid public game title and optional platform label to the existing live/planned schedule panel body, validates the full `StreamScheduleGameLink` before projection, exposes only bounded title/platform copy, and omits malformed optional game links without breaking the schedule. It adds no fetch and leaves CSS, layout, fallback copy, project behavior, and featured-Update behavior unchanged. Narrow checks pass with 159 Domain tests, 207 Web tests, Web typecheck/build, architecture, and diff checks. Full `pnpm check:review` remains a coordinator rerun. It is not deployed, live-verified, browser-verified, or rendered-fit-proven.
- The homepage current-project real-project integration is code-complete and independently reviewed after correction with zero standards and zero specification findings. It starts schedule and project loaders concurrently once each, validates the full public project summary contract, projects only title, summary capped at 280 characters, normalized slug, and optional milestone title, then chooses first active else first planning in API order. Completed-only is empty; malformed or unavailable data is unavailable. Narrow checks pass with 159 Domain tests, 182 Web tests, and Domain/Web typechecks. Full `pnpm check:review` remains a coordinator rerun. It is not deployed or browser/live-verified.
- The homepage featured-Update path tile integration is code-complete and independently reviewed with zero standards and zero specification findings. It starts schedule, project, and public Updates loaders concurrently once each, validates the full public Update summary contract, projects only normalized slug, title capped at 96 characters, and summary capped at 180 characters, and labels the first API-ordered update "Featured update", never latest. Empty and malformed/unavailable states stay distinct. Narrow checks pass with 159 Domain tests, 202 Web tests, Web typecheck/build, architecture, and diff checks. Full `pnpm check:review` remains a coordinator rerun. After commit and push, treat it as a deployment-gated homepage slice; it is not deployed, browser-verified, live-verified, or mobile-fit-proven yet.
