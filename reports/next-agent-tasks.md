...
- Phase E1 backup/export planning is captured in `reports/backup-restore-runbook.md`, and `pnpm dev:backup:health` now checks dev database reachability/core table presence through recurring smoke without exporting data. Production backup automation remains blocked on retention, encryption, restore-owner, dump-tool installation, and destructive-restore decisions.
