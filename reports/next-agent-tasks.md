...
- Phase 6G Twitch EventSub receiver: `POST /provider-webhooks/twitch/eventsub` verifies Twitch HMAC signatures using message id + timestamp + raw body, returns verification challenges, and logs notification/revocation payloads into `provider_event_intake_logs`. Provider status now reports `TWITCH_EVENTSUB_WEBHOOK_SECRET` readiness. No subscription creation, Twitch API calls, provider writes, public routing, money settlement, moderation enforcement, or production behavior was added.
- Phase 6G next: create/manage selected Twitch EventSub subscriptions for low-risk log-only events, then add Discord webhook and YouTube activity/pubsub surfaces. Start with read-only registration/logging and unknown-safe fallback rows; do not add provider writes, public routing, money settlement, moderation enforcement, or production behavior.
