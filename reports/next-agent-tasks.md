...
- The current follow-up adds compact service dots to `control-dev/chat` and hardens Twitch reconnect behavior. Twitch read-only chat intake now auto-reconnects after unexpected disconnects, stops auto-reconnecting after 10 disconnects in 10 minutes, and exposes a control-token reconnect action for the standalone chat window. YouTube/Discord remain honest disconnected indicators until their read-only intake phases are implemented.
...
- Phase 6F note: reconnect/autoreconnect and compact chat status are implemented in the current follow-up; remaining work is notification-panel alerting for repeated provider failures plus dev smoke.
