...
- Phase 6G YouTube PubSub receiver: local review-ready `GET/POST /provider-webhooks/youtube/pubsub` echoes hub verification challenges and logs Atom feed entries as `youtube-pubsub` provider intake rows using `fast-xml-parser`. No subscription creation, provider writes, public routing, money settlement, moderation enforcement, or production behavior was added.
- Phase 6G next: after review/deploy of the YouTube PubSub receiver, add YouTube PubSub subscription management and/or YouTube activities polling. Start with read-only registration/logging and unknown-safe fallback rows; do not add provider writes, public routing, money settlement, moderation enforcement, or production behavior.
