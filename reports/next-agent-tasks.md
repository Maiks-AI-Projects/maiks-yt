...
- Phase 6G follow-up: generated migration `0022_living_rage.sql` and domain normalization/redaction helpers are local for append-only `provider_event_intake_logs`. After review/application, add API/store runtime writes from existing Twitch/YouTube/Discord intake services. Store offline provider events by default, preserve provider payloads only in redacted/tightly bounded form, and route/display events only through explicit Event Routing rules.
