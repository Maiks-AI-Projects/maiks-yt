...
- Standalone/private streamer chat has source-tinted rows and first quick moderation controls. Hide, Ban, and Warn call universal Maiks.yt-local moderation endpoints for any source and can be retracted from the separate `/moderation` control window. Hide/ban persist through `moderation_active_states`, warnings persist through `moderation_audit_logs`, and API startup hydrates active rules into the chat filter. The third warning applies an automatic local stream-surface ban. Fake/local Note uses the existing fake/local moderation API. Discord, Twitch, and fail-closed YouTube provider warning delivery is implemented for provider-sourced messages with channel/user context and writes a separate provider-action audit row. Existing YouTube read-only credentials need re-consent for the newly requested write scope before live YouTube warning sends can work. Provider-side hide/delete/timeout/ban and censored-message allow controls remain later provider-write/allowlist slices.
...
- Phase B1 moderation follow-up adds visible disabled-action explanations to `/moderation` chat rows so missing local rights and provider-write gates are visible without changing authorization.
