...
- Michael wants moderators to have their own dedicated control window/PWA, not extra clutter in the creator chat window. First safe scope should combine permitted chat visibility, quick actions, active-rule review/retraction, pending approvals, and helper context behind existing moderator/helper permission gates.
  ...
  - Current implementation generated migration `0020_opposite_marauders.sql`, added `role_rank_paths`, added rank/path/promotion/Discord-role metadata to `roles`, extended dev seeds with owner/mod/admin defaults, and added owner-only rank path/role rights editing to `/admin/moderators`. Migration application and dev smoke are still pending.
