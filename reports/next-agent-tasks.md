...
- Phase 6A provider library capability mapping is deployed and dev-smoked. It now includes `@twurple/chat` and typed capability states in provider status. Twitch chat library availability is reported separately from runtime state, Twitch EventSub remains gated, and Phase 6C generated YouTube OAuth consent/token storage but has not been dev-applied yet. Phase 6D now adds `discord.js` and read-only Discord Gateway chat intake for coordinator review/deploy/smoke. YouTube live chat should stay last among the current chat providers.
...
- Phase 6D: Discord read-only Gateway message intake is implemented for review. Coordinator should deploy to dev, verify owner-gated admin status/start/stop, verify token-gated `/streamer-chat/discord-status`, send a harmless Discord message in the configured guild/channel, confirm it appears in the private streamer chat/control-panel feed, and confirm it does not appear on OBS overlay by default.
...
- Phase 6F note: reconnect/autoreconnect and compact chat status are implemented and dev-smoked; remaining work is notification-panel alerting for repeated provider failures.
