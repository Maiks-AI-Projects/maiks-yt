...
- Phase 6G YouTube activities poller: local review-ready owner-gated API/UI can manually poll recent selected-channel activities using the stored read-only YouTube credential and write projected `youtube-activity` rows into the intake ledger. This is a one-shot poll only. No recurring scheduler, provider writes, public routing, money settlement, moderation enforcement, or production behavior was added.
- Phase 6G next: review/deploy/smoke the manual YouTube activities poller. After that, provider intake foundations are broad enough to switch to the next explicit phase: provider-side moderation/write controls, recurring intake scheduling, or money/accounting, depending on Michael's priority.
