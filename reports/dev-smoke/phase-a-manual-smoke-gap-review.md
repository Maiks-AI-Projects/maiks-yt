...
- `/chat`: manual installed-window/browser smoke still needs a valid `control:open` token and should verify the standalone PWA scope, newest-first chat layout, service dots, token-blocked state, real private feed rendering, and that provider messages remain private and not overlay-routed by default.
- `/moderation`: manual installed-window/browser smoke still needs a valid control token plus a signed-in user with moderator/owner rights. Verify panel filtering, chat row actions, applied rules, retraction, pending approvals, live-helper summary, disabled-action copy, and emergency-clear gating.
...
- No browser automation or installed-window screenshots were captured in this pass.
