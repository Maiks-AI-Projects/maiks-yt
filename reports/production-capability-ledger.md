...
| Production route and operator-surface hygiene | done | done | in-progress | in-progress | done | in-progress | Revision `501613c` removes the dev test console, Gemini lab, retired Live Helper page, and fake overlay controls; web revision `986a48b` proves real HTTP `404` behavior; pushed commit `2b6264d` retires production dev identity/status exposure. A reviewed production patch now makes missing public runtime configuration fall toward production rather than dev origins | Deploy and verify the account/origin hygiene slices, then continue the narrower product-content audit one surface at a time |
