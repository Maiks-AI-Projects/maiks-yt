...
| Streaming-PC local agent and private audio | done | done | in-progress | not-started | not-started | not-started | `apps/local-agent` has tested outbound transport, durable dedupe/device identity, private cue/TTS execution, typed VLC/countdown extension points, and a fail-closed dedicated server WebSocket | Add sanitized owner status and credential rotation, implement VLC, then configure and prove the service on the streaming PC |
