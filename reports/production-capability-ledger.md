...
| Unified provider chat intake | done | done | done | in-progress | done | blocked | Deployed Twitch multi-channel, Discord, and YouTube intake code plus shared streamer-chat projection. Revision `2a99457` adds sanitized provider telemetry and owner controls that scope complete paginated EventSub listing/creation to an explicitly selected configured Twitch broadcaster. Current reviewed operator contracts replace raw provider-intake identifiers/payloads with owner-bound opaque review references and replace provider diagnostics with a finite readiness/capability/runtime projection. Connecting and retrying are distinct, retry timestamps are invariant-checked, and every Web consumer fails closed on malformed status. Commit `8e41696` adds bounded private-chat reconnect; these current patches are not deployed. Public and unauthenticated production smoke passed, but the owner workspace is not signed-in verified, no subscription mutation occurred, the bot token still needs revalidation, and real intake remains unproven | Deploy in a deliberate window, prove finite status against real runtimes plus owner-bound intake review/rollback against MariaDB, then prove installed reconnect/API restart behavior with real provider traffic; separately restore credentials and reconcile intended subscriptions |
