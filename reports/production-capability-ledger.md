...
| Production route and operator-surface hygiene | done | done | in-progress | in-progress | done | in-progress | Revision `501613c` removes the dev test console, Gemini lab, retired Live Helper page, and fake overlay controls; web revision `986a48b` proves real HTTP `404` behavior; pushed commits `2b6264d`, `8cb1ced`, `bc4d37f`, and `5144226` retire production dev identity/status exposure, dev-origin fallbacks, the inert Connections simulation source, stale implementation-detail copy, simulated Admin Overview state, and the inert AI tile. The current CSS-only patch removes the retired selectors after a source-consumer audit while preserving the Provider Integrations boundary list | Deploy and verify the accumulated hygiene slices, replace/unpublish example updates, and continue the operator-surface audit |
