...
| Unified provider chat intake | done | done | done | in-progress | done | blocked | Deployed Twitch multi-channel, Discord, and YouTube intake code plus shared streamer-chat projection; failed first-stream evidence shows the full path was not reliable | Prove MaiksPlays channel intake and reconnect behavior end to end before the next stream |
| Chat attention and viewer commands | done | done | done | in-progress | done | blocked | Deployed reliable attention readout and viewer commands; `!commands` exists in code | Verify live reply credentials, command output, unread attention, and suppression behavior on the actual channel |
...
| Production route and operator-surface hygiene | done | done | in-progress | in-progress | done | in-progress | Revision `501613c` removes the dev test console, Gemini lab, retired Live Helper page, and fake overlay controls; old paths render the production 404 experience, but streamed responses currently carry HTTP `200` | Correct dynamic not-found HTTP status, then continue the narrower product-content audit |
