...
| Unified provider chat intake | done | done | done | in-progress | done | blocked | Deployed Twitch multi-channel, Discord, and YouTube intake code plus shared streamer-chat projection; the next production revision adds sanitized runtime state without routine chat bodies/raw provider IDs, while current live evidence still shows the bot token returns `401`, three EventSub subscriptions target old `maiksmc`, and the intake ledger has no rows | Deploy and owner-verify the provider telemetry correction, then restore the live bot credential and channel targets before rehearsal |
...
| Event routing and alerts | done | done | done | in-progress | done | blocked | Deployed real-provider execution, cooldown/approval history, top/center transport, and alert-sound work; production code now routes newly inserted Twitch, Discord, and YouTube chat ledger rows with stable actor identity and duplicate suppression | Deploy the chat-intake correction, then rehearse real follow/sub/chat-derived events, dedupe, audio ownership, and fallback in OBS |
...
| PWA session and rapid recovery | done | done | in-progress | in-progress | done | blocked | Bounded transient retry is deployed and reacts within 30 seconds plus online/foreground; revision `f12c98d` enforces `requiresLogin` on Control HTTP and WebSocket paths and includes session cookies on all Control PWA API calls; a valid bare token returns `401` live; API revision `4bb517d` is deployed and live logs retain paths without query details; signed-in installed recovery is not yet proven and the dedicated OAuth return page awaits image approval | Rotate the previously exposed verification token, approve and implement the recovery page, then prove durable installed access in less than one minute |
