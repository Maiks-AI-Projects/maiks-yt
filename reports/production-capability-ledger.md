...
| Remaining static runtime/bootstrap credentials | done | done | done | done | done | done | Ten scoped KV units now cover all identified application credentials. Exact source/render/container equality passed without value output; OAuth/provider/database/auth checks remained stable; every credential line is absent from \`.env.production\`; ordinary configuration remains there. The expired Twitch bot bundle remains truthfully \`needs_attention\` and awaits separate reauthorization. |
| Version-controlled Agent render contract | done | done | done | done | done | done | \`scripts/openbao/production-api.env.ctmpl\` records only the allowlisted key/path mapping. Focused tests reject unknown paths/keys, bootstrap or admin material, and literal secret assignments. The live render remains ignored and mode \`0600\`. |
| Better Auth OAuth/session data protection | in-progress | done | in-progress | not-started | not-started | not-started | Four provider account rows and three current sessions still use ordinary MariaDB token columns. The active audit must preserve account behavior and session continuity, cover direct token readers and future provider credential writes, and leave hash-only URL access tokens unchanged. |
| Normal deployment and backup continuity | done | done | done | done | done | done | Standard Compose consumed the ignored Agent render, recreated only API on the exact image, and preserved companion services and volumes. The encrypted snapshot timer also fired naturally and exited successfully. |
...
| Public writing style | done | done | not-applicable | not-applicable | not-applicable | done | Michael selected `Direct, warm, and matter-of-fact` at `https://choices.mmc.onl/p/maiks-yt/l/df2127362d9157df`. The choice guides page-by-page writing and has been recorded in the production decision file; it does not authorize a blind rewrite. |
| Dedicated public AI-use page | done | done | done | done | done | done | The dedicated-page choice superseded the paragraph-only placement idea. Commit `127d1a3218d336938be66b255f87e7158a1bb003` implemented the approved copy, was independently reviewed, deployed on image config `sha256:d1fd3479c7d5586dfbcb18d0065500ed837237214b1cc6771d15425cd6345860`, and was verified through the public HTTP route. |
...
| Public profiles and recognition | done | done | in-progress | not-started | not-started | not-started | `/profiles` is still a static public/private demonstration, not live profile behavior. Michael selected `/profiles/maiks`, one-year retirement before handle reuse, and manual Owner assignment of first handles to existing accounts. Private-account searchability and its name-only/no-image projection remain approved. Commit `a752c67` contains the independently reviewed Domain contract. The production MariaDB preflight and Owner-only audit design are complete. Choice `fc1c6e1363e3d9f6` selected `Generate files`; commit `fcf1c8f` generated the reviewed schema, `0030_certain_abomination.sql`, snapshot, and journal entry. Nothing was applied or deployed, and no reservation, assignment, runtime consumer, or live profile exists. The first Profiles candidates are rejected. The replacement navigation/account/profile proposal and ten-image set await exact Owner visual approval. Recognition, linked identities, perks, and verified game names still lack approved public persistence or consent rules. | Obtain exact Owner approval for the replacement visual set. Separately complete the backup/application gate before applying `0030`, assigning handles, exposing provider data, or changing accounts. |
| Public AI-use disclosure | done | done | done | done | done | done | Michael selected the dedicated page and note `use a dedicated page`. The approved direct, warm, matter-of-fact copy and image-first references are recorded under `reports/production-about-ai-page-proposal.md` and `reports/visual-concepts/production-about-ai/`. Commit `127d1a3218d336938be66b255f87e7158a1bb003` implemented the page, was independently reviewed, deployed on image config `sha256:d1fd3479c7d5586dfbcb18d0065500ed837237214b1cc6771d15425cd6345860`, and was verified through the public HTTP route. It added no AI runtime behavior. | Keep the page copy bounded to the approved disclosure. Any future AI feature or broader navigation change needs its own delivery record. |
...
| Event routing and alerts | done | done | done | done | done | in-progress | Deployed real-provider execution, cooldown/approval history, top/center transport, alert sounds, durable provider-chat routing, broadcaster-scoped Twitch live/offline resolution, and configured-channel EventSub controls. Revision `09383126b4d7874576689bb7cd0588639954f3b2` completes production approval execution with an opaque browser reference, pending-row compare-and-set ownership, idempotent matching retries, current website-consent recheck, top/center-only playback, status-only `approval_queue`, and explicit browser projection allowlists. It is deployed to API and Web on provenance-labelled image `sha256:0a765f9e...a9d4`; both are healthy with zero restarts, public/protected HTTP contracts are finite, and Control/Overlay plus persistent volumes were preserved. The production admin catalogue omits simulated/test-only entries and rejects their save/reset/delete operations before persistence while retaining the non-production harness and all real kinds. | Prove one naturally received real non-money event queues without playback, one authorized approval emits once to an active overlay, a matching retry does not emit again, and live MariaDB concurrency plus sanitized signed-in responses hold. Continue schedule/provider/cooldown/audio/OBS rehearsal separately. |
| Music catalog, requests, and playback | done | done | done | in-progress | done | blocked | Deployed music schema/API/UI, YouTube Audio Library import, browser/OBS fallback, per-PWA output selection, and server-side projection to the VLC agent. Commit `eb421e8` expires abandoned Local Agent commands exactly once and releases an unacknowledged VLC lease for browser fallback. Commit `6913f63`, deployed through `8e3bc62`, reduces successful public request responses to `{ ok: true, accepted: true }`. Revision `e5f0dd8` removes anonymous track/source ids and private provider/license/policy metadata, introduces opaque exact-selection references, re-resolves eligibility inside the daily-request transaction, and adds strict finite Web parsing without changing private account Top 10, admin, playback, Local Agent, VLC, review, or persistence identifiers. Revision `55a8991`, deployed through `aed2a1c`, removes raw Track/Source/Policy relationship-id entry from Music Catalog authoring. Revision `7edc5e8`, with runtime behavior in `ed76923`, now also removes raw Track/Source entry and saved-row display from Music Review, adds readable relationship selection, finite stale/authorization/concurrency behavior, and preserves private API ids. Independent reviews and `pnpm check:review` pass; Web is healthy on provenance-labelled image `sha256:89ed7810...1607`, while API/Control/Overlay and persistent volumes were preserved. The live catalog is empty and no request was written merely for verification. The streaming-PC agent remains unconfigured and unverified. | Populate the real catalog and prove authenticated Top 10 plus owner review, including the readable Review selectors. Then configure the local agent and prove real catalog playback, command expiry, automatic next, `stream_music`, reconnect, history, and browser fallback. |
...
| Production route and operator-surface hygiene | done | done | done | done | done | in-progress | Production removes the dev test console, Gemini lab, retired Live Helper page, fake overlay controls, dev identity/status exposure, dev/auth/API-origin fallbacks, inert simulation UI, stale implementation-detail copy, simulated Admin Overview state, retired CSS, legacy fake overlay/moderation injection routes, and unauthenticated realtime spike routes. Admin Overview no longer consumes development smoke state; public Updates omit raw ids/fixture markers; production Event Routing omits and rejects simulated/test catalogue state; finite Admin Overview and Page Creator copy are deployed. Revision `aed2a1c` removes unsupported Sponsor and fundraising Goal controls from production Control and omits both production mutation routes while preserving dormant compatibility state; live POST checks return ordinary `404`. Real HTTP `404` behavior is verified. | Continue the response-contract audit and verify signed-in operator routes do not expose retired or development-only states. |
