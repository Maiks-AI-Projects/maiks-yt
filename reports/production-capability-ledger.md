...
| Event routing and alerts | done | done | done | in-progress | done | blocked | Deployed real-provider execution, cooldown/approval history, top/center transport, and alert sounds; the next production revision adds durable-chat routing plus broadcaster-scoped Twitch live/offline resolution with EventSub/Helix ordering, coalescing, and fail-closed unknown state | Deploy the reviewed routing slices, verify EventSub targets and real live/offline transitions, then rehearse follow/sub/chat events, dedupe, audio ownership, and OBS fallback |
