...
| Operational Chat, Moderation, Control, and Notifications PWAs | done | done | done | in-progress | done | in-progress | Deployed at `501613c` with the Control auth correction deployed at `f12c98d`; local desktop, half-screen, touch, and mobile evidence is in `reports/visual-qa/production-pwa-redesign/README.md`; routes and manifests are deployed, but installed-window and signed-in owner-session proof is still absent and the dedicated recovery page is pending visual approval | Verify installed access, real provider data, moderation effects, music output, and overlay controls |
