...
| Public profiles and recognition | done | done | in-progress | not-started | not-started | not-started | `/profiles` is explicitly a static public/private demonstration. Michael selected `/profiles/maiks`, one-year retirement before handle reuse, and manual Owner assignment of first handles to existing accounts. Private-account searchability and its name-only/no-image projection remain approved. The canonical one-table persistence proposal passed senior review for a domain-rules-only stage. Domain normalization, transition decisions, and minimized public/private projection rules are implemented and under final review. No migration exists. Recognition, linked identities, perks, and verified game names still lack approved public persistence or consent rules. | Finish the Domain review, then complete the MariaDB, audit, reservation-action, image-route, backup, and migration gates before generating or applying schema or changing live accounts. |
| Public AI-use disclosure | done | in-progress | not-started | not-started | not-started | not-started | Michael selected a short dedicated public AI-use page and rejected placing the proposed section on the other About pages, with note `use a dedicated page`. The direct, warm, matter-of-fact 215-word copy proposal and desktop/mobile image-first candidates are registered under `reports/production-about-ai-page-proposal.md` and `reports/visual-concepts/production-about-ai/`. Image-generated body text is representative; the proposal remains the exact-copy authority. | Michael reviews the desktop/mobile candidates and exact copy. Do not implement before that bounded approval. |
| Unified provider chat intake | done | done | done | in-progress | done | blocked | Deployed Twitch multi-channel, Discord, and YouTube intake code plus shared streamer-chat projection. Revision `2a99457` adds sanitized provider telemetry and broadcaster-scoped EventSub controls. Revision `30adb56` deploys a finite `twitch_chat_replies` capability with bounded cached read-only validation and exact Web parsing. Public smoke and the unauthenticated owner gate pass; a sanitized live API-runtime probe returns `needs_attention / invalid_access_token`. The owner workspace, real intake, and installed reconnect path remain unverified | Reauthorize the Twitch bot credential with the required IRC chat scopes, then prove signed-in finite status, installed reconnect/API restart behavior, owner-bound intake review, and real provider traffic; separately reconcile intended subscriptions |
...
| Music catalog, requests, and playback | done | done | done | in-progress | done | blocked | Deployed music schema/API/UI, YouTube Audio Library import, browser/OBS fallback, per-PWA output selection, and server-side projection to the VLC agent. Commit `eb421e8` expires abandoned Local Agent commands exactly once and releases an unacknowledged VLC lease for browser fallback; its API source is deployed through `30adb56`, while the streaming-PC agent remains unconfigured and unverified. | Configure the local agent, then prove real catalog playback, command expiry, automatic next, `stream_music`, reconnect, history, and browser fallback. |
...
| PWA session and rapid recovery | done | done | done | done | done | blocked | Bounded retry and `requiresLogin` enforcement are deployed. Revision `2a99457` generates production create/rotate launch URLs for `control.maiks.yt`/`overlay.maiks.yt` while dev stays isolated, without changing hash-only storage or the login gate. Reviewed source commit `b686792` adds the approved recovery flow with the exact existing website logo, a recovery-only public header, purpose-built authenticated PWA chrome, allowlisted returns, pre-render token stripping, configured providers, and a clean signed-out Notifications renewal path. Independent review and `pnpm check:full` pass. This new source is not yet pushed, deployed, or installed-PWA verified. | Push and deploy Web/API/Control with rollback tags, then prove real OAuth return, provider availability, signed-in create/rotate, revoked-token behavior, and installed recovery in less than one minute. |
