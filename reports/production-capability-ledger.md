...
| Public profiles and recognition | done | in-progress | in-progress | not-started | not-started | not-started | `/profiles` is explicitly a static public/private demonstration. Private-account searchability and its name-only/no-image projection are approved. Choice list `https://choices.mmc.onl/p/maiks-yt/l/e5b5dc8c09ec15a8` asks for Michael's permanent profile address, retired-handle reuse, and existing-account handle assignment before a normalized-handle migration proposal. Recognition, linked identities, perks, and verified game names still lack approved public persistence/consent rules. | Michael submits the profile-address choices; then prepare the schema and migration proposal for review without applying it. |
...
| PWA session and rapid recovery | done | in-progress | in-progress | in-progress | done | blocked | Bounded retry and `requiresLogin` enforcement are deployed. Revision `2a99457` now generates production create/rotate launch URLs for `control.maiks.yt`/`overlay.maiks.yt` while dev stays isolated, without changing hash-only storage or the login gate. The compact production-style OAuth return candidate is registered at `reports/visual-concepts/production-pwa-access-recovery/access-recovery-candidate-v1.png` and awaits Michael's approval. Signed-in create/rotate and installed recovery remain unproven. | Michael approves or corrects the recovery-page image; then implement the exact approved flow, owner-verify launch URL generation, rotate the previously exposed verification token, and prove installed recovery in less than one minute. |
