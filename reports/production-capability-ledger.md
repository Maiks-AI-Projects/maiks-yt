...
| Production route and operator-surface hygiene | done | done | in-progress | in-progress | done | in-progress | Revision `501613c` removes the dev test console, Gemini lab, retired Live Helper page, and fake overlay controls; web revision `986a48b` proves real HTTP `404` behavior; pushed commits through `ff5718e` retire production dev identity/status exposure, dev-origin and auth-origin fallbacks, the inert Connections simulation source, stale implementation-detail copy, simulated Admin Overview state, the inert AI tile, and retired CSS. The current API patch also removes the last production-capable localhost public-origin fallbacks from account/runtime URL generation | Deploy and verify the accumulated hygiene slices, replace/unpublish example updates, and continue the operator-surface audit |
