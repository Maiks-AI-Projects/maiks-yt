...
| PWA session and rapid recovery | done | done | in-progress | in-progress | done | blocked | Bounded transient retry is deployed and reacts within 30 seconds plus online/foreground; revision `f12c98d` enforces `requiresLogin` on Control HTTP and WebSocket paths and includes session cookies on all Control PWA API calls; a valid bare token now returns `401` live, but signed-in installed recovery is not yet proven and the dedicated OAuth return page awaits image approval | Redact URL-token query parameters from logs, rotate the verification token, approve and implement the recovery page, then prove durable installed access in less than one minute |
...
6. Redact URL access-token query parameters from production request logs and rotate any token used during live verification.
