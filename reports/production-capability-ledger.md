...
| Production route and operator-surface hygiene | done | done | in-progress | in-progress | done | in-progress | Revision `501613c` removes the dev test console, Gemini lab, retired Live Helper page, and fake overlay controls; web revision `986a48b` proves real HTTP `404` behavior; pushed commits through `799dd76` retire production dev identity/status exposure, dev/auth/API-origin fallbacks, the inert Connections simulation source, stale implementation-detail copy, simulated Admin Overview state, the inert AI tile, and retired CSS. The reviewed Control patch removes orphaned realtime/simulator UI and the simulation-only Moderation approvals route, aligns shell access with real remaining capabilities, and passes the production review gate | Push the Control cleanup, then deploy and verify the accumulated hygiene slices, replace/unpublish example updates, and continue the operator-surface audit |
