...
| Account identity and linked login providers | done | done | done | done | done | in-progress | Focused account pages, independent Maiks.yt name/image, linked provider choices, and production auth. Reviewed production commit `2b6264d` removes legacy development identity/status routes, replaces the Connections dependency with a signed-in minimal provider-id projection, and corrects avatar privacy caching; it is pushed but not deployed | Deploy the reviewed hygiene patch, then verify sign-in, provider listing/linking, profile replacement/privacy, and session recovery on the live origin |
...
| Production route and operator-surface hygiene | done | done | in-progress | in-progress | done | in-progress | Revision `501613c` removes the dev test console, Gemini lab, retired Live Helper page, and fake overlay controls; web revision `986a48b` proves real HTTP `404` behavior; pushed commits `2b6264d`, `8cb1ced`, and `bc4d37f` retire production dev identity/status exposure, dev-origin fallbacks, the inert Connections simulation source, and stale implementation-detail copy. The current production patch also removes simulated approvals, fake/local moderation state, and the inert AI tile from Admin Overview while retaining real operator health evidence | Deploy and verify the accumulated hygiene slices, replace/unpublish example updates, remove only proven-dead CSS, and continue the operator-surface audit |
