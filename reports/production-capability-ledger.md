...
| Music catalog, requests, and playback | done | done | done | in-progress | blocked | blocked | Production music schema/API/UI, YouTube Audio Library import, browser/OBS playback, per-PWA output selection, and a tested but disconnected local-agent foundation | Establish the real catalog, verify browser playback, then add the dedicated agent credential/server path and VLC execution while retaining fallback |
| Streaming-PC local agent and private audio | done | done | in-progress | not-started | not-started | not-started | `apps/local-agent` has tested outbound transport, durable dedupe/device identity, private cue/TTS, executable `stream_music` VLC control, a countdown extension point, and a fail-closed dedicated server WebSocket | Map Maiks.yt playback and authenticated media to VLC, add sanitized owner status/rotation, then configure and prove the service |
