...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Public site and public content | done | done | done | done | done | in-progress | Revision `501613c`; live `200` smoke across the main public routes; owner Games has a read-only reverse Schedule summary, and collapsed reviewed-history filters are corrected with focused regression coverage. Reviewed production slices omit explicitly marked example Updates from production list, detail, RSS, and the ordinary owner inventory. Michael approved the image-first `/admin/updates` direction; the owner editor and authenticated API now provide draft create/edit, exact saved preview, opaque revision-safe publish, idempotent unpublish/publish retry behavior, and no audit identifiers. Commit `f457085` restores currently live visible entries to public schedule reads without exposing completed/private rows. The current reviewed permissions patch also excludes revoked and expired grants from project-admin authorization while preserving active delegated access. Neither patch is deployed | Deploy in a deliberate verification window, then prove the signed-in owner publishing lifecycle, stale-preview rejection against MariaDB, public detail/RSS visibility, unpublish behavior, live schedule visibility, owner Games with real schedule data, and active-versus-inactive delegated project permissions |
...
| Event routing and alerts | done | done | done | in-progress | done | blocked | Deployed real-provider execution, cooldown/approval history, top/center transport, alert sounds, durable provider-chat routing, broadcaster-scoped Twitch live/offline resolution, and configured-channel EventSub controls through revision `2a99457`. The reviewed production patch also connects public schedule create/update/cancel to real website rule resolution with atomic history/approval/cooldown decisions, hashed cooldown identity, private-schedule silence, and isolated playback failures; that patch is not deployed. Service and auth-gate smoke passed, but owner configuration, schedule-to-overlay delivery, live MariaDB concurrency, and real transition/event rehearsal remain unverified | Commit and deploy the schedule-event bridge only in a deliberate verification window, then prove real schedule history/routing/overlay behavior and cooldown transaction behavior; separately owner-verify provider targets and rehearse follow/sub/chat events, dedupe, audio ownership, and OBS fallback |
