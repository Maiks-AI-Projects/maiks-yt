...
| Public profiles and recognition | done | done | in-progress | not-started | not-started | not-started | `/profiles` is explicitly a static public/private demonstration. Michael selected `/profiles/maiks`, one-year retirement before handle reuse, and manual Owner assignment of first handles to existing accounts. Private-account searchability and its name-only/no-image projection remain approved. Commit `a752c67` contains the independently reviewed Domain contract for normalization, reserved-name policy, atomic transitions, safe public identifiers, and minimized public/private projections. `pnpm check:review` passes. The production MariaDB preflight is complete and the proposal pins user-reference collations to match `users.id`. `reports/profile-handle-audit-event-store-proposal.md` now closes the separate design gate with an independently reviewed Owner-only operation/detail model, atomic canonical/audit commits, append-only enforcement requirements, finite replay/rejection behavior, and no public/browser raw identity. No schema, migration, runtime consumer, reservation, or live profile exists. Recognition, linked identities, perks, and verified game names still lack approved public persistence or consent rules. | Michael chooses whether to generate migration files at `https://choices.mmc.onl/p/maiks-yt/l/fc1c6e1363e3d9f6`. Application, append-only mechanism proof, retention/pseudonymization decisions, the `maiks` data action, image routing, backup/restore proof, API/UI integration, and live account changes remain separate. |
