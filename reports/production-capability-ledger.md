...
| Public profiles and recognition | done | done | in-progress | not-started | not-started | not-started | `/profiles` is explicitly a static public/private demonstration. Michael selected `/profiles/maiks`, one-year retirement before handle reuse, and manual Owner assignment of first handles to existing accounts. Private-account searchability and its name-only/no-image projection remain approved. Commit `a752c67` contains the independently reviewed Domain contract for normalization, reserved-name policy, atomic transitions, safe public identifiers, and minimized public/private projections. `pnpm check:review` passes. The production MariaDB preflight is complete and the proposal pins user-reference collations to match `users.id`; no migration or runtime consumer exists. Recognition, linked identities, perks, and verified game names still lack approved public persistence or consent rules. | Resolve audit storage, the `maiks` reservation data action, image routing, backup/restore proof, and the separate migration-generation gate before adding schema, API/UI integration, or changing live accounts. |
