...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Public site and public content | done | done | done | done | blocked | blocked | Production routes, typed public loaders, and `pnpm check:review`; owner Games now has a read-only reverse Schedule summary with local responsive proof; public origin currently returns Cloudflare `530` | Restore production reachability, identify the deployed revision, then smoke the main public and owner Games routes |
...
| Backup, export, and recovery evidence | done | in-progress | in-progress | in-progress | blocked | blocked | Key-data export, backup health with sanitized failure categories, runbook, and a dry-run record exist | Approve retention/encryption/owner policy, install the production workflow, and prove a disposable restore drill |
...
| Production route and operator-surface hygiene | done | done | in-progress | in-progress | blocked | blocked | The production web build no longer registers the dev test console, Gemini lab, or retired Live Helper page; old public links to `/dev` and `/gemini-lab` fail closed; the Control PWA no longer exposes disabled future overlay actions; static profile demonstrations and some implementation-detail copy remain | Deploy and verify the retired routes and cleaned operator surfaces after reachability returns, then continue the narrower product-content audit |
