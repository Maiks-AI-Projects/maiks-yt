...
| Page Creator and route ownership | done | done | done | in-progress | done | blocked | Deployed persistence, owner editor, preview/publish gate, reserved routes, and exact-path rendering. The current reviewed contract patch keeps audit identity and route ownership server-side while exposing only the editor/publication fields the owner browser uses | Deploy the minimized contract, then verify live draft isolation and publish/unpublish behavior before using it for production pages |
