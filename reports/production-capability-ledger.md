...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Public site and public content | done | done | done | done | done | in-progress | Revision `501613c`; live `200` smoke across the main public routes; owner Games has a read-only reverse Schedule summary, and collapsed reviewed-history filters are corrected with focused regression coverage. The current reviewed slice omits explicitly marked example Updates from production list, detail, and RSS data while retaining honest non-production fixtures | Deploy the content-integrity boundary, then verify owner Games with a real signed-in owner session and real schedule data |
