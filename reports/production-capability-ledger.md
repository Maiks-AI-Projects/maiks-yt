...
| Unified provider chat intake | done | done | done | in-progress | done | blocked | Deployed Twitch multi-channel, Discord, and YouTube intake code plus shared streamer-chat projection. Revision `2a99457` adds sanitized provider telemetry and owner controls that scope complete paginated EventSub listing/creation to an explicitly selected configured Twitch broadcaster. The current reviewed operator-contract patch replaces raw provider-intake browser identifiers and payloads with an exact allowlisted projection plus owner-bound opaque review references; internal-history mapping is transactional and strict reference parsing fails closed. Commit `8e41696` adds bounded private-chat reconnect, one active socket, process-session/revision ordering, malformed-frame HTTP recovery, and missed-message reconciliation; it is not deployed. Public and unauthenticated production smoke passed, but the owner workspace is not signed-in verified, no subscription mutation occurred, the bot token still needs revalidation, and real intake remains unproven | Deploy in a deliberate window, prove owner-bound intake review and rollback against MariaDB, then prove installed reconnect/API restart behavior with real provider traffic; separately restore credentials and reconcile intended subscriptions |
