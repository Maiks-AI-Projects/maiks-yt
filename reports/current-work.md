...
- Reviewed and accepted Phase 4A locally: safe Event Routing playback projection, owner-gated pending approval list/review endpoints, direct safe simulated top/center overlay playback, and `/admin/event-routing` pending-review controls. Internal-only and unsafe playback paths fail closed; real provider intake, real website production dispatch, real money, moderation enforcement, provider integrations, secrets, migrations, and server state are still gated until coordinator deploy/smoke finishes.

## Current Task

Deploy and dev-smoke Phase 4A Event Routing playback, then choose the next follow-up: community operations or another bounded feature lane.
...
3. Phase 3 stream-visibility consent is live on dev; Phase 4A adds safe simulated approval/direct overlay playback pending deployment. Future real website dispatch still needs production-safe intake, event templates, provider intake, moderation boundaries, and real-money gates before any production stream output.
