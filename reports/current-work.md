...
Assign the first runtime-safe Event Routing implementation now that the reviewed persistence migration is applied on dev.
...
3. Event Routing persistence migration `0012_smooth_jack_flag.sql` is applied on dev; the next safe slice is manual/provider-neutral routing-rule admin plus safe simulated/test dispatch only.
