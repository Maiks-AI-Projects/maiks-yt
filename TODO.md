...
- [ ] Provider integration gate: define provider scopes, failure handling, rate limits, token storage, revocation, and manual override before Twitch/YouTube/Discord/music/payment integrations.
  ...
  - 2026-07-04 typed provider-event/action catalog, append-only intake ledger migration, runtime writes for existing read-only chat intake services, read-only admin intake visibility, broad Discord Gateway non-chat intake logging, and the first Twitch EventSub verified/log-only receiver are implemented locally. The next gate is creating/managing selected Twitch EventSub subscriptions or adding Discord webhook/YouTube activity/pubsub intake, still without provider writes or public routing.
