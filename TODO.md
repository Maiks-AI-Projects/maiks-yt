...
- [ ] Provider integration gate: define provider scopes, failure handling, rate limits, token storage, revocation, and manual override before Twitch/YouTube/Discord/music/payment integrations.
  ...
  - 2026-07-04 typed provider-event/action catalog and generated append-only intake ledger migration are implemented locally. The next gate after migration review/application is API/store runtime writes from existing Twitch/YouTube/Discord intake services, still without provider writes or public routing.
