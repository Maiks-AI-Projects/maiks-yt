...
Note: 2026-06-21 added an in-code `@maiks-yt/domain/events` registry for dev-console planning only. Durable event routing rules, event history, per-user opt-outs, cooldown state, provider credentials, moderation enforcement, and real/simulated money persistence remain a future schema gate.
  ...
  - 2026-06-29 Phase 6A added the first real provider SDK foundation in `@maiks-yt/integrations` for Twitch/Twurple, YouTube/googleapis, and Discord REST, plus sanitized provider configuration status and owner-gated `/admin/provider-integrations/status` with `/admin/provider-integrations` display. Missing env vars now report safe missing status and secret values are never returned. OAuth, token storage/rotation, webhooks/EventSub receivers, live chat ingestion, provider moderation/write actions, real money, migrations, deployments, Cloudflare/Docker config, auth flow changes, and production behavior remain gated.
...
- [ ] Provider integration gate: define provider scopes, failure handling, rate limits, token storage, revocation, and manual override before Twitch/YouTube/Discord/music/payment integrations.
  - 2026-06-29 Phase 6A opened only the read-only provider SDK/config-status foundation. The full provider integration gate remains open for scopes, rate limits, durable token storage, revocation, manual override, webhook/chat intake safety, provider failure handling, and production-secret readiness before any real provider intake or action.
