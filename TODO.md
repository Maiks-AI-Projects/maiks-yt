...
- [x] Persist fake/local active moderation state after durable active-state migration.
  - 2026-06-29 Phase 5I wires fake/local hide and temporary mute commands into `moderation_active_states` and adds a read-only active fake/local moderation summary to `/admin/live-helper`. Warn, note, and noop commands remain audit-only. Rows are forced fake-local, test, simulated, resettable, and `providerAction: false`; live suppression still uses the in-memory runtime cache for immediate fake/local behavior. No provider enforcement, destructive actions, durable provider state, money/support authority, AI decisions, auth changes, secrets, or production behavior was added.
