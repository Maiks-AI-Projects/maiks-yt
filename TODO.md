...
- [x] Harden every supporting Provider Integrations browser contract, move YouTube OAuth URL construction behind an owner-gated same-origin launcher, use owner-bound opaque channel references, and deploy the reviewed Web/API slice with exact runtime provenance (`96a5db7`).
...
- [x] Embed immutable OCI source revision/source metadata in the normal production image build so runtime provenance is artifact-verifiable (Tracker issue `47a660c41b580ccc`; completed by build-gate commit `bba9d3a08de5ef70e93587b4f2f57aabd90820dc`, with live proof at production revision `42b30651bbd38b854705fcd552929a7e3e49472f`).
...
Note: 2026-06-21 added an in-code `@maiks-yt/domain/events` registry for dev-console planning only. Durable event routing rules, event history, per-user opt-outs, cooldown state, provider credentials, moderation enforcement, and real/simulated money persistence remain a future schema gate.
  ...
  - [x] Feed authoritative Twitch live/offline stream state into rule execution and fail closed while state is unknown. Provider intake continues while offline; this state controls routing/display only.
    - 2026-08-27 production code resolves Twitch state per originating broadcaster through ordered EventSub observations plus read-only Helix, with short-lived cache, token/identity/state coalescing, and fail-closed unknown handling.
  - [ ] Rehearse real broadcaster mapping and live/offline transitions before enabling the admin `When` control. Other providers and `oncePerStream` remain unresolved.
  - [x] Connect normalized real Twitch, YouTube, and Discord events to Event Routing rule resolution and destination execution after durable intake. Duplicate and failed intake writes do not route, and routing failures do not trigger provider retries.
  - [ ] Connect reviewed production website events to Event Routing. Enforce persisted visibility opt-outs, actor identity, cooldown prerequisites, and privacy/internal-only boundaries before any public destination executes.
    - 2026-08-28 production code connects public schedule create/update/cancel mutations to real website Event Routing through atomic history, approval, and cooldown decisions. Private schedules and game-link-only edits remain silent; schedule routing failures do not roll back the saved schedule. Account/profile events and project-update publication still need separate producer-specific review, so the parent item remains open.
    - 2026-08-28 schema gate: `website.project-update-published` is for Project updates in `project_updates`, not generic public Updates posts, recaps, or announcements. It should emit only when a Project update changes from not publicly readable to publicly readable: create-as-published under a public eligible Project, draft to published while visible on a public eligible Project, hidden to visible while published on a public eligible Project, a Project visibility/status change that makes existing published visible updates public, or a later genuine re-publish after unpublish. Idempotent retries and edits to already-public updates must stay silent. Current second-precision `project_updates.published_at` and append-only `event_history.source_event_id` do not give a durable producer occurrence identity for same-second re-publish or visibility-driven publication. Future work needs a coordinator-approved publication occurrence/outbox design, likely a `project_update_publication_events`-style record with a monotonic sequence and unique deterministic source event id, before implementation or migration.
  ...
  - 2026-08-27 production follow-up makes the owner EventSub manager explicitly multi-channel aware. It preserves legacy primary-channel precedence, allows an operator to select another valid configured broadcaster such as `maiksplays`, rejects malformed or unconfigured targets before Twitch API/token work, and completes bounded Helix pagination before broadcaster-scoped list/default evaluation. This code is independently reviewed and locally verified but not deployed; no subscription was created or changed. Owner UI verification, explicit per-channel subscription reconciliation, and real intake proof remain open.
...
- [x] Define protocol v1 for a single authenticated Maiks.yt OBS bridge and coordinate-free widget snapshots.
  ...
  - [x] Complete and deploy the production approval execution path for queued real non-money events. Approval now uses an opaque browser reference and a pending-row compare-and-set, publishes at most once to the existing top/center overlay consumers, keeps `approval_queue` status-only, rechecks current website visibility consent, makes matching retries idempotent, and keeps raw queue/history/rule ids plus playback projections server-side. A naturally received provider/overlay rehearsal remains separate.
...
- [x] Apply the first reviewed homepage concept as the production homepage.
  - 2026-08-28 production code connects the homepage stream-status and "Up next" slots to the real public schedule read model. It fetches once, shares one public-safe projection, prefers current live entries with earliest-start tie-breaks, otherwise chooses the earliest valid planned `startsAt` regardless of API array order, omits cancelled/completed entries, and keeps loaded-empty distinct from unavailable or malformed schedule data. Revision `bd8e8de` is deployed, and live public API/Home verification proves the honest loaded-empty schedule state.
  - 2026-08-28 production code optionally appends the first ordered valid public scheduled-game focus to the existing live/planned schedule panel body. It validates the full `StreamScheduleGameLink` before use, exposes only bounded game title and optional platform label, omits malformed optional links without breaking the schedule, adds no fetch, and leaves fallback copy, layout, CSS, project behavior, and featured-Update behavior unchanged. The deployed live schedule is currently empty, so a real planned/live game-focus rendering remains a later data-dependent verification.
  - 2026-08-28 production code connects the current-project card to the real public project list. The homepage starts schedule and project loaders concurrently once each, validates the full `PublicProjectSummary` contract, then passes only title, summary capped at 280 characters, normalized slug, and optional milestone title into the existing card. It chooses first active else first planning in API order, treats completed-only as empty, and treats malformed/error data as unavailable. Revision `bd8e8de` is deployed, and live verification proves the authoritative active `Community Onboarding Notes` project renders on Home.
  - 2026-08-28 production code connects the existing "Projects and updates" path tile to the first authoritative API-ordered public Update as "Featured update", never latest. The homepage starts schedule, project, and public Updates loaders concurrently once each, validates the full `PublicUpdateSummary` contract, and passes only normalized slug, title capped at 96 characters, and summary capped at 180 characters into the tile. Revision `bd8e8de` is deployed, and live verification proves the honest loaded-empty featured-Update state.
...
- [ ] Redesign the linked public schedule, projects, updates, links, games, rules, and privacy pages one at a time.
  ...
  - [x] Harden Better Auth environment defaults: production falls back only to production Web/Control/Overlay origins and `api.maiks.yt`, while local/dev origins remain non-production-only and explicit environment values keep precedence. Deployment remains separate.
  - [x] Make the remaining `API_PUBLIC_BASE_URL` fallbacks environment-aware in account image URLs, domain identity, and API runtime construction; explicit configuration keeps precedence.
  ...
  - [x] Unpublish backend records explicitly marked as examples from production update lists, detail routes, and RSS data while retaining labelled examples outside production for development.
...
- [x] Define roles and permissions.
  ...
  - 2026-08-29 production cleanup removed inactive Discord role-id authoring and raw audit identifier fallbacks from `/admin/moderators` while preserving stored mappings and all operational rank/grant behavior. Scope and availability are still persistence metadata rather than enforced authority, and role/rank-path changes still need a durable authority audit; both remain explicit production issues before this model can be called complete.
...
- [x] Build a dedicated moderator control window.
  - Michael wants moderators to have their own window where they can assist across the fronts they are allowed to manage. The production window combines private chat visibility, permission-gated quick actions, active rule review/retraction, user context, and audit without exposing owner-only provider credentials, money, auth, deployment, or destructive controls. A future real approval workflow belongs behind its own decision capability, not the retired simulated Live Helper summary.
  ...
  - 2026-07-02 added the first chat-first `/moderation` control window. The production cleanup now retains permission-filtered Chat, Active Moderation, Applied Rules, User Context, and Audit while removing the simulated Pending Approvals/Live Helper summary. Stream chat hide/ban/warn/retract mutations require both a valid control URL token and a signed-in user with matching role rights, and emergency clear requires `chat:emergency-clear`.
...
- [x] Add website admin pages for catalog, playlists, keep/restrict/reject/blacklist review, license metadata, preview playback, and played history.
  ...
  - 2026-08-29 `/admin/music/review` replaces raw Track/Source relationship-id entry and display with readable selections and saved-row labels. Relationship scopes validate required selections, stale and mismatched rows fail closed without leaking ids, transient refresh errors preserve the last useful view while authorization loss clears it, and overlapping refreshes are latest-request-wins.
...
- [x] Add separate `/music/player` browser/audio source as the first production playback consumer.
  - 2026-08-30 production repair enabled public requests on the existing allowed YouTube Audio Library policy after proving the only stored track, source, current CC BY 4.0 snapshot, local file, and checksum were eligible. The anonymous and signed-in catalogs now truthfully contain Hackerland; no fixture or weakened eligibility rule was introduced.
  - The deployed OBS browser fallback was also restored from `-100 dB` to `0 dB`. With streaming and recording off, an existing stream-audio scene changed the private monitor from `-91.0 dB` silence to measured program audio after Michael's signed-in Pause/Resume control; the original scene was restored and playback was left paused. The pre-change OBS snapshot is private at `/home/michael/.local/state/maiks-yt-backups/music-repair-20260830T1949Z/obs-before.json`.
...
- [x] Add sanitized owner-visible local-agent device status.
  - Owner-only `GET /admin/local-agent/status` reports configuration readiness, connection freshness, service version, advertised module actions/availability, and bounded VLC playback state. The existing Admin Overview shows a compact connection/module row. Both omit credentials, device and agent identifiers, command/acknowledgement data, raw module details, paths, and playback IDs.
...
- [x] Connect authoritative Maiks.yt playback to local VLC while retaining `/music/player` as a fallback.
  - Maiks.yt remains authoritative for selection, play/pause/skip, history, review outcomes, and now-playing state. The local connector claims playback only while its VLC capability is available, downloads audio with the dedicated bearer without placing it in URLs or VLC arguments, reports started/ended/error lifecycle state, advances to the next track, targets `stream_music`, and releases the browser fallback lease on disconnect or command failure. Local Agent deployment, service installation, VLC audio-channel proof, and reconnect rehearsal remain open; browser fallback proof is recorded below.
  - 2026-08-27 expiring Local Agent commands now produce one terminal `COMMAND_EXPIRED` acknowledgement and clear pending state. An unacknowledged `track.play` releases the `local-agent-vlc` lease for browser fallback; commands without an expiry remain unchanged. Real VLC verification remains open; browser fallback verification is recorded below.
  - 2026-08-30 the browser/OBS fallback is live-verified end to end with the real stored track, signed-in owner controls, `200` control mutations, authenticated media delivery, and measured non-silent output. The Local Agent/VLC path remains a separate gate because the streaming-PC service and its dedicated least-privilege device credential are not configured; owner, dev-auth, provider, or overlay credentials were not substituted.
...
- [x] Add authoritative music controls to the existing Control PWA instead of creating a separate music PWA.
  - The `/control/music` view reads real playback state and sends permission-gated play, pause/resume, and skip commands. It preserves the `/music/player` browser fallback while the local VLC agent is unavailable. Signed-in owner Pause/Resume and real browser/OBS audio are verified in production; real Local Agent/VLC playback remains open.
