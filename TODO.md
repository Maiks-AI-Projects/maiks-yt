...
- [x] Design game library, game suggestions, and play-schedule links.
  ...
  - 2026-08-15 generated migration `0027_jazzy_odin.sql` and implemented the first on-demand provider-neutral catalog cache. Explicit admin searches plus Steam owned/wishlist previews cache sanitized canonical/provider metadata; stale rows remain searchable during Steam failure; remote artwork URLs stay internal and image files are not downloaded. `/admin/games` now uses a searchable title combobox with keyboard navigation, free-text fallback, and owner-confirmed catalog linking.
  - 2026-08-15 coordinator review, dev migration application, and live API smoke completed. Steam search returned cached catalog results, owned and wishlist previews loaded, and catalog/provider identity rows were written without importing personal library entries or exposing the catalog publicly. Authenticated browser interaction remains a manual check because the available QA browser sessions were signed out.
  - 2026-08-15 added public Steam store links, cached safe icon URLs, and real current-player counts. Public games sort by the cached Steam count with manual order/title fallback; provider failure keeps the last good cache and never blocks the public page. Migration `0028_real_layla_miller.sql` adds the nullable popularity cache fields. Legacy Steam rows without a catalog link can resolve the same cached metadata by exact canonical Steam store URL.
  - Future catalog work: add Twitch category/IGDB lookup and an explicit owner merge/match editor. Do not invent a YouTube per-game ID, bulk crawl provider catalogs, or overwrite manual game-library metadata.
