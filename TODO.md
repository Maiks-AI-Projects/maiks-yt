...
- [ ] Provider integration gate: define provider scopes, failure handling, rate limits, token storage, revocation, and manual override before Twitch/YouTube/Discord/music/payment integrations.
  ...
  - 2026-07-04 typed provider-event/action catalog, append-only intake ledger migration, runtime writes for existing read-only chat intake services, read-only admin intake visibility, and broad Discord Gateway non-chat intake logging are implemented locally. The next gate is Twitch EventSub receiver plumbing for verified/log-only events, still without provider writes or public routing.
