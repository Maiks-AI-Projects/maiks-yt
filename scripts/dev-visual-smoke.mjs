...
const buildSurfaceList = ({ adminUrl, controlUrl, chatUrl, overlayUrl, webBaseUrl }) => {
  ...
  if (adminUrl) {
    for (const [name, path, expectedText] of [
      ...
      ["admin-testing", "/admin/testing", ["Testing Guide", "Quick Open", "Stream Windows", "Installed Window Checklist", "Manual Testing Checklist", "Session started", "Start new session", "Copy progress", "Reset marks", "Mark section done", "Clear section", "Session Notes", "Import Preview", "duplicate_reference", "possible_duplicate", "Testing note", "Copy template", "Severity: blocking / annoying / polish"]],
